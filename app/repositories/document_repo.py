...
from app.schemas.response import DocumentProcessingResponse, DocumentRecordSummary, DocumentListResponse

logger = logging.getLogger("neostats.document_repo")

class DocumentRepository:
    @staticmethod
    def save_document_record(response: DocumentProcessingResponse) -> int:
        """
        Saves full processed document record to database.
        """
    ...
    def get_latest_document_by_name(document_name: str) -> Optional[Dict[str, Any]]:
        """
        Retrieves latest document processing result by document_name.
        """
        conn = get_db_connection()
        cursor = conn.cursor()
    ...
    def list_documents(limit: int = 50, offset: int = 0, document_type: Optional[str] = None) -> DocumentListResponse:
        """
        Retrieves paginated document summaries.
        """
        conn = get_db_connection()
        cursor = conn.cursor()
